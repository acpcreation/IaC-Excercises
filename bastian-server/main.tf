...
locals {
  # For now, use the variable. Security group extraction can be added later with proper provider support
  final_ssh_cidrs = var.allowed_ssh_cidr_blocks
}
...
resource "aws_security_group" "bastion" {
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = local.final_ssh_cidrs
  }
